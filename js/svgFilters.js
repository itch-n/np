...
/**
 * Intended to make unvisited parks look like holes in the map surface by replacing
 * the cream fill with a very dark brown. In practice the emblems rendered as solid
 * black discs - the inset shadow detail was lost against the dark fill. Keeping this
 * here for reference but the active filter remains inset-shadow.
 */
export function createHoleShadowFilter(defs, config) {
  const filter = defs.append('filter')
    .attr('id', 'hole-shadow');

  filter.append('feFlood')
    .attr('flood-color', '#1C1008')
    .attr('result', 'darkBg');
  ...
  const merge = filter.append('feMerge');
  merge.append('feMergeNode').attr('in', 'darkShape');
  merge.append('feMergeNode').attr('in', 'outerShadow');
